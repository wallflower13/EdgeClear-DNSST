...
def _ssim_3d_tensor(img1, img2, max_value=1.0):
    """使用PyTorch计算SSIM

    Args:
        img1 (Tensor): [B, C, H, W] 格式的图像
        img2 (Tensor): [B, C, H, W] 格式的图像
        max_value (float): 图像的最大值
    ...
    # 创建高斯核
    x = torch.arange(kernel_size, device=img1.device).float() - kernel_size // 2
    kernel_1d = torch.exp(-x.pow(2) / (2 * sigma ** 2))
    kernel_1d = kernel_1d / kernel_1d.sum()

    # 创建2D内核
    kernel = kernel_1d.view(-1, 1) * kernel_1d.view(1, -1)
    kernel = kernel.view(1, 1, kernel_size, kernel_size).repeat(img1.size(1), 1, 1, 1)

    pad = kernel_size // 2

    # 计算均值
    mu1 = F.conv2d(img1, kernel.to(img1.device), groups=img1.size(1), padding=pad)
    mu2 = F.conv2d(img2, kernel.to(img2.device), groups=img2.size(1), padding=pad)
    ...
    # 计算方差和协方差
    sigma1_sq = F.conv2d(img1 ** 2, kernel.to(img1.device), groups=img1.size(1), padding=pad) - mu1_sq
    sigma2_sq = F.conv2d(img2 ** 2, kernel.to(img2.device), groups=img2.size(1), padding=pad) - mu2_sq
    sigma12 = F.conv2d(img1 * img2, kernel.to(img1.device), groups=img1.size(1), padding=pad) - mu1_mu2
...
def PSNR(pred, target):
    ...
    return 20 * torch.log10(torch.tensor(1.0, device=pred.device) / torch.sqrt(mse))
...
def SSIM(pred, target):
    ...
    return _ssim_3d_tensor(pred, target)
